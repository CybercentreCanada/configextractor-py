...
class Extractor:
    def __init__(
        self, framework, module, module_path, root_directory, yara_rule, venv=None
    ) -> None:
...
class Framework:
    ...
    def extract_yara_from_module(
        self, decoder: object, module_name: str, existing_rule_names=[]
    ) -> List[str]:
        if (
            self.yara_attr_name
            and hasattr(decoder, self.yara_attr_name)
            and getattr(decoder, self.yara_attr_name)
        ):
            yara_rules = list()
    ...
    def run(
        self, sample_path: str, parsers: Dict[Extractor, List[yara.Match]]
    ) -> Dict[str, dict]:
    ...
    def run_in_venv(self, sample_path: str, extractor: Extractor) -> Dict[str, dict]:
        ...
        with NamedTemporaryFile(
            "w", dir=os.path.dirname(extractor.module_path), suffix=".py"
        ) as script:
            with NamedTemporaryFile("w") as yara_rule:
                ...
                with NamedTemporaryFile() as output:
                    ...
                    script.flush()
                    custom_module = (
                        script.name.split(".py")[0]
                        .replace(f"{extractor.root_directory}/", "")
                        .replace("/", ".")
                    )
                    proc = subprocess.run(
