...
class MACO(Framework):
    ...
    def extract_yara(self, parsers: List[str]) -> Tuple[List[str], List[str]]:
        ...
        for parser_path in parsers:
            if not self.validate_parsers([parser_path]):
                continue

            decoder = MACO.get_parser_class(parser_path)()
    ...
    def validate_parsers(self, parsers: List[str]) -> List[str]:
        ...
        for parser_path in parsers:
            ...
            try:
                parser = SourceFileLoader(parser_name, parser_path).load_module()
                if (hasattr(parser, 'Extractor') and parser.Extractor == Extractor) or \
                    (hasattr(parser, 'extractor') and parser.extractor.Extractor == Extractor) or \
                        (hasattr(parser, 'maco') and parser.maco.extractor.Extractor == Extractor):
